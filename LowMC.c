...
void mpc_xor4_3(uint32_t* temp[3], uint32_t* in[4][3], lowmcparams_t* params)
{
    ...
    for (uint8_t i = 0; i < 3; i++) {
        for (uint32_t j = 0; j < params->stateSizeWords / 2; j++) {
            temp_64[i][j] = in1_64[i][j] ^ in2_64[i][j] ^ in3_64[i][j]
                            ^ in4_64[i][j] ^ temp_64[i][j];
        }
    }
}
    ...
    for (i = 0; i < params->stateSizeWords; i++) {
        for (size_t j = 0; j < 4; j++) {
            for (size_t k = 0; k < 2; k++) {
                bits_vec[j][k] = linearMatricesLookupTableVec[round][i * (sizeof(uint32_t)) + j][(state[k][i]
                                                                                                  >> (24 - (8 * j))) & 0xFF];
            }
        }
...
void mpc_multiplyWithKeyMatrix2(uint32_t* key[2], uint32_t round, __m256i output[2], lowmcparams_t* params)
{
    ...
    for (i = 0; i < params->stateSizeWords; i++) {
        for (size_t j = 0; j < 4; j++) {
            for (size_t k = 0; k < 2; k++) {
                bits_vec[j][k] = keyMatricesLookupTableVec[round][i * (sizeof(uint32_t)) + j][(key[k][i]
                                                                                               >> (24 - (8 * j))) & 0xFF];
            }
        }
    ...
    for (i = 0; i < params->stateSizeWords; i++) {
        for (size_t j = 0; j < 4; j++) {
            for (size_t k = 0; k < 3; k++) {
                bits_vec[j][k] =
                    linearMatricesLookupTableVec[round][i * (sizeof(uint32_t)) + j][(state[k][i]
                                                                                     >> (24 - (8 * j))) & 0xFF];
            }
        }
...
void mpc_multiplyWithKeyMatrix(uint32_t* key[3], uint32_t round, __m256i output_vec[3], lowmcparams_t* params)
{
    ...
    for (i = 0; i < params->stateSizeWords; i++) {
        for (size_t j = 0; j < 4; j++) {
            for (size_t k = 0; k < 3; k++) {
                bits_vec[j][k] =
                    keyMatricesLookupTableVec[round][i * (sizeof(uint32_t)) + j][(key[k][i]
                                                                                  >> (24 - (8 * j))) & 0xFF];
            }
        }
